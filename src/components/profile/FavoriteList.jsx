...
export default function FavoriteList({ onRecipeClick }) {
  ...
  const handleRecipeClick = (recipeId, category) => {
    if (onRecipeClick) onRecipeClick(recipeId, category);
  };

  // Pisahkan resep berdasarkan kategori
  const makanan = favorites.filter((r) => r.category === "makanan");
  const minuman = favorites.filter((r) => r.category === "minuman");
